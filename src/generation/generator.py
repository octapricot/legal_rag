...
def _clean_llm_json(raw: str) -> str:
    ...
    # Remove ALL control characters except \n \r \t (the only valid JSON whitespace)
    text = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]', ' ', text)
    # Collapse runs of spaces that may result from the above
    text = re.sub(r'  +', ' ', text)
...
# ---------------------------------------------------------------------------
# Fallback renderer (when JSON parsing fails)
# ---------------------------------------------------------------------------
...
def _fallback_brief(
    ...
    """
    When the LLM produces malformed JSON, render a clean brief directly
    from the retrieved chunks rather than showing raw error output.
    """
    today = date.today().isoformat()
    guideline_names = list({c.source_name for c in retrieved_chunks if c.doc_type == "guideline"})
    ...
    lines += [
        ...
        "",
        "> *Note: The AI model returned a response that could not be fully structured. "
        "The retrieved source passages are listed below for manual review.*",
        "",
    ...
    for chunk in retrieved_chunks:
        lines.append(f"### {chunk.citation_key}")
        lines.append(f"*{chunk.full_name}*")
        lines.append("")
        lines.append(f"> {chunk.text[:600]}{'...' if len(chunk.text) > 600 else ''}")
        lines.append("")
...
def generate_brief(
    ...
    try:
        ...
        # JSON parsing failed — try to salvage what we can from the raw response
        print(f"  JSON parse error: {e}")
        return _fallback_brief(query, raw_response, retrieved_chunks)
